...
{
    if (row < 0 || row > 1) return; // hanya 2 baris: 0 dan 1

    lcd_put_cur(row, 0);  // Pindah ke awal baris
    for (int i = 0; i < 16; i++)  // Hapus seluruh baris dengan spasi
    {
        lcd_send_data(' ');
    }
    lcd_put_cur(row, 0);  // Kembalikan kursor ke awal baris
}
